...
The identity system is hierarchical. The main bot identity sits at the root, and agent/subagent identities are nested within the bot's memory directory. Any prompts must be saved as text files (see `config/agents/_shared/` and per-agent directories under `config/agents/`) to minimize prompt injection and allow for secure, auditable prompt management.
