//! `araliya-memory` — session management and pluggable storage for the Araliya bot.
//!
//! This crate owns the memory subsystem: session lifecycle, pluggable store
//! backends, optional SQLite-backed document stores (feature-gated), and the
//! `MemoryBusHandler` that exposes `memory/*` methods on the supervisor bus.
//!
//! ## Feature flags
//!
//! | Flag           | Purpose                                           |
//! |----------------|---------------------------------------------------|
//! | `isqlite`      | Enables the general-purpose `SqliteStore`          |
//! | `idocstore`    | Adds `IDocStore` (FTS5 document index) + manager   |
//! | `ikgdocstore`  | Adds `IKGDocStore` (docstore + knowledge graph)    |
