...
pub(super) async fn handle_message(
    ...
    let reply_result = tokio::time::timeout(
        ...
        state.send_message(
            channel_id,
            msg_req.message.clone(),
            requested_session_id,
            msg_req.agent_id.clone(),
        ),
    )
    .await;
...
pub(super) async fn handle_session_detail(
    ...
    let result = tokio::time::timeout(
        ...
        state.request_session_detail(session_id, None),
    )
    .await;
...
pub(super) async fn handle_session_memory(
    ...
    let result = tokio::time::timeout(
        Duration::from_secs(10),
        state.request_session_memory(session_id, None),
    )
    .await;
...
pub(super) async fn handle_memory_agent_kg(
    ...
    let result =
        tokio::time::timeout(Duration::from_secs(10), state.request_memory_kg(agent_id)).await;

    match result {
        Ok(Ok(data)) => super::write_json_response(socket, "200 OK", data.as_bytes()).await,
        Ok(Err(e)) => {
            warn!(%channel_id, %agent_id, "memory KG request failed: {e}");
            let err_body = serde_json::json!({
                "error": "not_found",
                "message": format!("{e}")
            });
        ...
        Err(_) => {
            let err_body = serde_json::json!({
                "error": "timeout",
                "message": "memory KG request timed out"
            });
            super::write_json_response(
                socket,
                "504 Gateway Timeout",
...
pub(super) async fn handle_session_files(
    ...
    let result = tokio::time::timeout(
        Duration::from_secs(10),
        state.request_session_files(session_id, None),
    )
    .await;
...
pub(super) async fn handle_llm_providers(
    socket: &mut tokio::net::TcpStream,
    state: &Arc<CommsState>,
    channel_id: &str,
) -> Result<(), AppError> {
    let result = tokio::time::timeout(Duration::from_secs(10), state.request_llm_providers()).await;
    match result {
        Ok(Ok(data)) => super::write_json_response(socket, "200 OK", data.as_bytes()).await,
        Ok(Err(e)) => {
...
pub(super) async fn handle_llm_set_default(
    socket: &mut tokio::net::TcpStream,
    state: &Arc<CommsState>,
    channel_id: &str,
    body: Vec<u8>,
