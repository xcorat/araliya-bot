...
use araliya_core::bus::{
    BusError, BusHandle, BusPayload, BusResult, ComponentInfo, ComponentStatusResponse,
    CronScheduleSpec, ERR_METHOD_NOT_FOUND,
};
use araliya_core::bus::{BusHandler, HealthReporter, SubsystemHealth};

use crate::service::{CronCommand, CronService};
            ...
            tokio::spawn(async move {
                let h = match reporter {
                    Some(r) => r
                        .get_current()
                        .await
                        .unwrap_or_else(|| SubsystemHealth::ok("cron")),
                    None => SubsystemHealth::ok("cron"),
                };
