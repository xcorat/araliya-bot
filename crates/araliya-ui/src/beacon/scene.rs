...
fn settings_centre() -> (f64, f64) {
    ctrl_centre(1)
}
fn ui_centre() -> (f64, f64) {
    ctrl_centre(2)
}
fn close_centre() -> (f64, f64) {
    ctrl_centre(3)
}
...
pub fn hit_test(x: f64, y: f64, controls_visible: bool) -> Hit {
    if controls_visible {
        ...
        if hit_hex(cx, cy, CTRL_R, x, y) {
            return Hit::Close;
        }
        let (ux, uy) = ui_centre();
        if hit_hex(ux, uy, CTRL_R, x, y) {
            return Hit::Ui;
        }
        let (sx, sy) = settings_centre();
        if hit_hex(sx, sy, CTRL_R, x, y) {
            return Hit::Settings;
        }
    }

    if hit_hex(HEX_CX, HEX_CY, MAIN_R, x, y) {
        return Hit::MainHex;
    }
    Hit::Nothing
}
...
fn draw_ctrl(scene: &mut Scene, (cx, cy): (f64, f64), kind: CtrlKind) {
    ...
    scene.fill(
        Fill::NonZero,
        Affine::IDENTITY,
        Color::from_rgba8(12, 14, 26, 210),
        None,
        &path,
    );
    let (stroke, icon) = match kind {
        CtrlKind::Close => (
            Color::from_rgba8(220, 80, 80, 220),
            Color::from_rgba8(220, 80, 80, 200),
        ),
        CtrlKind::Ui => (
            Color::from_rgba8(80, 200, 255, 220),
            Color::from_rgba8(80, 200, 255, 200),
        ),
        CtrlKind::Settings => (
            Color::from_rgba8(160, 100, 255, 220),
            Color::from_rgba8(160, 100, 255, 200),
        ),
    };
